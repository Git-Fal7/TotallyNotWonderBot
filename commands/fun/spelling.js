...
module.exports.run = async (bot, message, args) => {
    const type = require('../json/spelling.json');
    const item = type[Math.floor(Math.random() * type.length)];
    const filter = response => {  
      return item.spell.some(answer => answer.split("").join(' ').toLowerCase() === response.content.toLowerCase());
    };
    let Image = Canvas.Image,
    canvas = new Canvas.createCanvas(1065, 442),
    ctx = canvas.getContext('2d');
    let background = await Canvas.loadImage('./commands/images/spelling.png');
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
    ctx.font = "mitix 69px AGA Arabesque";
    ctx.fontSize = '20px';
    ctx.fillStyle = "#f1f1f1";
    ctx.textAlign = "center";
    ctx.fillText(item.spell, 530, 247)
