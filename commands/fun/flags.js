...
module.exports.run = async (bot, message, args) => {
    const type = require('../json/flags.json');
    const item = type[Math.floor(Math.random() * type.length)];
    const filter = response => {  
      return item.flag.some(answer => answer.toLowerCase() === response.content.toLowerCase());
    };
    let Image = Canvas.Image,
    canvas = new Canvas.createCanvas(1065, 442),
    ctx = canvas.getContext('2d');
    let background = await Canvas.loadImage('./commands/images/flags.png');
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
    ctx.font = "mitix 69px AGA Arabesque";
    ctx.fontSize = '20px';
    ctx.fillStyle = "#f1f1f1";
    ctx.textAlign = "center";
    ctx.fillText(item.country, 530, 247)
    ...
    message.channel.send(attachment).then(() => {
      message.channel.awaitMessages(filter, { maxMatches: 1, time: 15000, errors: ['time'] })
        ...
        .catch(collected => { // Expected when a user never answered, any error in the code will go into this too
           message.channel.send(`You couldn't type the country flag correctly in time.. :(`);
        })
      });
}
